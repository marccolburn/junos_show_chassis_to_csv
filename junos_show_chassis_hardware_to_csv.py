...
def dump_json_to_csv(json_data):
    ...
    output_file = "hardware_parts.csv"
    if not os.path.exists(output_file):
        with open(output_file, mode='x') as hardware_file:
            schema = ['Name', 'Description', 'DeviceSN','ParentSN']
            writer = csv.writer(hardware_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow([g for g in schema])
    with open(output_file, mode='a') as hardware_file:
        ...
        chassis_id = json_data["chassis-inventory"][0]["chassis"][0]["serial-number"][0]["data"]
...
if __name__ == "__main__":
    ...
    if os.path.isdir(source_dir):
        ...
        print ("\n\n" + (source_dir) + " does not exist.\nPlease place your json files in " + (source_dir) + " and run again\n\n")
